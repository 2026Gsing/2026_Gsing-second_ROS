...
if ! command -v convert &> /dev/null; then
    ...
    sudo apt update && sudo apt install -y imagemagick ros-${_DETECTED_ROS2}-nav2-map-server ros-${_DETECTED_ROS2}-nav2-bringup ros-${_DETECTED_ROS2}-rviz2 > /dev/null 2>&1
fi
