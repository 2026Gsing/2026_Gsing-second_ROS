...
Preprocess::Preprocess() : feature_enabled(0), lidar_type(AVIA), blind(0.01), point_filter_num(1), z_limit_enable(false), z_min(-1e9), z_max(1e9)
{
  inf_bound = 10;
  N_SCANS = 6;
  SCAN_RATE = 10;
...
void Preprocess::oust64_handler(const sensor_msgs::msg::PointCloud2::UniquePtr &msg)
{
  ...
  if (feature_enabled)
  {
    ...
    for (uint i = 0; i < plsize; i++)
    {
      ...
      if (z_limit_enable && (added_pt.z < z_min || added_pt.z > z_max))
        continue;
...
void Preprocess::default_handler(const sensor_msgs::msg::PointCloud2::UniquePtr &msg)
{
  ...
  // Find time / ring field offsets in the raw PointCloud2
  int time_off = -1, ring_off = -1;
  for (const auto &f : msg->fields)
  {
    if (f.name == "time")  time_off = f.offset;
    if (f.name == "ring")  ring_off = f.offset;
  }
  int pt_step  = msg->point_step;
  ...
  for(uint i = 0; i < plsize; ++i)
  {
    ...
    // Extract point timestamp from raw binary data (avoid PCL type-mapping pitfalls)
    if (time_off >= 0)
    {
      float t;
      memcpy(&t, raw + i * pt_step + time_off, sizeof(float));
      added_pt.curvature = t * time_unit_scale;
    }
    else
    {
      added_pt.curvature = 0.;
